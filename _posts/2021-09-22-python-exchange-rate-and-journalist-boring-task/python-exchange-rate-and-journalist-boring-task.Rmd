...
So I built a simple program in Python to make life slightly easier. 
...
Now, we can start converting the base figure, rounding it and writing it in a format that is easier to read. We will also store the symbol of the currency output that we will use later.

```{python}
# The currency symbol takes the opposite currency of the currency input
if base_currency == "IDR":
    result = currency_input / exchange_rate
    currency_symbol = "US$"
else:
    result = currency_input * exchange_rate
    currency_symbol = "Rp "

if result > 1000000 and result < 1000000000:
    result = round(result / 1000000, 2)
    unit = "million"
elif result > 1000000000 and result < 1000000000000:
    result = round(result / 1000000000, 2)
    unit = "billion"
elif result > 1000000000000 and result < 1000000000000:
    result = round(result / 1000000000000, 2)
    unit = "trillion"
elif result > 1000000000000000:
    result = round(result / 1000000000000000, 2)
    unit = "quadrillion"
else:
...
```{python}
result = f"{currency_symbol}{result:,} {unit}"
...
The result is a very simple yet helpful program. It allows users to convert rupiah figures to US dollars or the other way around all in the command line.
...
But programming languages can also give us journalists a small win, such as improving the workflow for converting currencies.